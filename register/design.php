...
if(isset($_POST['Register']))
    {
        ...
        if (empty($age)){
            $ageErr = 'Please submit your date of birth.';
           }
          
           # (2) Use regular expresstion to validate date format
           elseif (!preg_match('~^([0-9]{2})/([0-9]{2})/([0-9]{4})$~', $age, $parts)){
            $ageErr= 'The date of birth is not a valid date in the format MM/DD/YYYY';
           }
          
           # (3) Use checkdate to check if day, month and year are properly defined.
           elseif (!checkdate($parts[1],$parts[2],$parts[3])){
            $ageErr = 'The date of birth is invalid. Please check that the month is between 1 and 12, and the day is valid for that month.';
            if (empty ($ageErr))
        ...
        if (empty($file)){
            $fileErr = "Please Enter File";
        } 
        if(!$fnameErr && !$lnameErr && !$emailErr && !$ageErr && !$phoneErr && !$addressErr && !$fileErr ){
    ...
    <body>
        <form name="form1" method="post" action="design.php">
            <table align="center" >
                ...
                <tr>
                    <td>Age</td>
                    <td><input type ="date" name="age" placeholder="age" value="<?php if(isset($age)){echo $_SESSION['age'];}?>"/>
                ...
                <tr>
                    <td>File</td>
                    <td><input type="file"name="file"id="file" value="<?php if(isset($file)){echo $_SESSION['file'];}?>" >
                    <span class="text-danger"><?php if (isset($fileErr)){echo $fileErr;}?></span></td>
                </tr>
